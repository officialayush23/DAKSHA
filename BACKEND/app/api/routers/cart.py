...
from app.services.session_service import get_active_session, start_session
...
def quick_add_to_cart(
    ...
    """
    UI shortcut: add a product card item directly to cart without needing
    a session_id from the frontend. Auto-resolves the user's active session.
    """
    try:
        session = get_active_session(db, user_id=user.id)
        if not session:
            session = start_session(db, user_id=user.id, channel=ChannelEnum.web)
        ...
        db.commit()
        return {"status": "added", "variant_id": str(payload.variant_id)}
    except ValueError as e:
        db.rollback()
        raise HTTPException(status_code=400, detail=str(e))
