...
class LoyaltyService:
    ...
    def get_loyalty_summary(user_id: str):
        """Returns points balance and eligible rewards."""
        user = supabase.table("users").select("loyalty_points, loyalty_tier").eq("id", user_id).single().execute()
        if not user.data:
            return None
            
        points = user.data['loyalty_points']
        
        # Calculate Reward Eligibility (Simple Logic)
        rewards = []
        if points >= 500:
