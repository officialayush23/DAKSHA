...
from app.agents.tools_fulfillment import (
    schedule_fulfillment_tool,
    get_fulfillment_status_tool,
)
from app.agents.tools_notifications import send_notification_tool
...
# ---- Helper: create worker node, like before ----
def create_worker_node(tools, system_prompt: str):
    ...
    def worker_node(state: AgentState):
        ...
        # Log agent run (best-effort, no crash if fails)
        try:
...
inv_node, inv_tool_node = create_worker_node(
    inv_tools,
    "InventoryAgent. You answer 'Is it in stock?' and 'Which store has it?'."
)
...
pay_node, pay_tool_node = create_worker_node(
    pay_tools,
    "PaymentAgent. You manage cart, apply promotions (via checkout), and confirm orders."
)
...
# Fulfillment (NEW)
ful_tools = [schedule_fulfillment_tool, get_fulfillment_status_tool, track_order_tool]
ful_node, ful_tool_node = create_worker_node(
    ful_tools,
    "FulfillmentAgent. You schedule delivery/pickup/reservation and keep user updated on fulfillment."
)

# (Optionally, Notification Agent could be separate, but for now Notifications are just tools used by others.)
...
supervisor_system_prompt = (
    ...
    "Use:\n"
    "- RecommendationAgent: discovery, styling, bundles, 'what should I buy?'\n"
    "- InventoryAgent: availability, sizes, nearest store, in-store navigation.\n"
    "- PaymentAgent: cart changes, apply promo at checkout, payment issues before order is created.\n"
...
async def run_sales_agent(user_id: str | None, channel: str, channel_id: str, message: str):
    ...
    Handles omni-session upsert and passes context into the graph.
    """
    # Get or create omni session
    ...
    # Update omni-session summary (very simple)
