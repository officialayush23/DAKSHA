...
export default function Checkout() {
  ...
  const { refreshCart, user, endSession } = useKiosk();
  ...
  const handleRemove = async (variantId) => {
    try {
      await KioskService.removeCartItem(variantId);
      fetchCart(); // Refresh local list
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      // 2. Simulate Payment Delay (Terminal Interaction)
      await new Promise(resolve => setTimeout(resolve, 3000));

      // 3. Confirm Payment (In real app, this happens via webhook)
      // API: POST /payment/pay/{checkout_id}
  ...
  return (
    <div className="flex h-[calc(100vh-80px)] bg-slate-50">
      ...
      <div className="w-2/3 p-8 overflow-y-auto">
        ...
        {loading ? (
          <div className="h-64 flex items-center justify-center"><Loader2 className="h-12 w-12 animate-spin text-slate-300"/></div>
        ) : cartItems.length === 0 ? (
