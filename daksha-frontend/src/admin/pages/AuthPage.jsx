  ...
  const handleAuth = async (e) => {
    ...
    try {
      ...
      if (isRegister) {
        ...
        // ---------------------------------------------------------
        // ✅ ADDED: Print JWT Token to Console for Debugging
        // ---------------------------------------------------------
        if (result.data?.session) {
            console.log("Login Successful! JWT Token:", result.data.session.access_token);
        }

        toast.success("Welcome back to Daksha");
        
        // NOTE: If this is for the Admin Panel, change '/shop' to '/admin/dashboard'
        navigate('/admin');
      }
    } catch (err) {
